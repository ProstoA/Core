    ...
    public class HideField : IDataModelFix {
        ...
        public IEnumerable<IDataModel> Applay(IEnumerable<IDataModel> items) {
            foreach (var item in items) {
                ...
                var constraints = (DataConstraints)fieldType.GetTypeInfo().GetProperty("Constraints").GetValue(item);
                constraints = (DataConstraints)fieldType.GetTypeInfo().GetMethod("Clone").Invoke(item, new object[0]);
